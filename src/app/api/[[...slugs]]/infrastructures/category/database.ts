...
import { Category, CreateCategory, UpdateCategory } from '../../domain';
...
@injectable()
export class CategoryDatabaseRepositoryImpl implements CategoryDatabaseRepository {
  constructor(
    @inject("DatabaseRepository")
    private readonly database_repo: DatabaseRepository
  ) { }
  async create(cat: CreateCategory): Promise<Category> {
    ...
    const query = `
      INSERT INTO ${this.database_repo.getKeyspace()}.Categories (
